-- Track how each donation was received or distributed.

do $$
begin
  create type public.donation_method as enum ('online', 'cash', 'other');
exception
  when duplicate_object then null;
end $$;

alter table public.donations_in
add column if not exists method public.donation_method not null default 'online';

alter table public.donations_out
add column if not exists method public.donation_method not null default 'online';

drop view if exists public.community_donations_in;
drop view if exists public.community_donations_out;

create or replace view public.community_donations_in as
select
  di.id,
  case
    when public.is_admin() or di.user_id = auth.uid() then di.user_id
    else null
  end as user_id,
  di.donated_at,
  di.amount_cents,
  di.method,
  di.reference_id,
  di.status,
  di.created_at,
  di.updated_at,
  case
    when public.is_admin() or di.user_id = auth.uid() then di.document_id
    else null
  end as document_id,
  di.user_id = auth.uid() as is_own,
  case
    when public.is_admin() or di.user_id = auth.uid() then p.username
    else null
  end as donor_username,
  p.reference_id as donor_reference_id
from public.donations_in di
join public.profiles p on p.id = di.user_id
where di.deleted_at is null;

grant select on public.community_donations_in to authenticated;

create or replace view public.community_donations_out as
select
  id,
  donee_name,
  donated_at,
  amount_cents,
  method,
  reference_id,
  status,
  created_at,
  updated_at
from public.donations_out
where deleted_at is null;

grant select on public.community_donations_out to authenticated;
